...
const COPY: Record<string, {
  ...
  ru: {
    ...
    h1sub: "Вы видите всё. В реальном времени.",
    sub: "Собственная ITSM-платформа ARKANA. Каждая заявка, каждый актив, каждый показатель SLA — у вас перед глазами. Никаких «разберёмся» и «позвоните завтра».",
  ...
  en: {
    ...
    h1sub: "Full visibility. In real time.",
    sub: "ARKANA's own ITSM platform. Every ticket, every asset, every SLA metric — visible to you at all times. No more \"we'll look into it\" or \"call back tomorrow\".",
  ...
  uz: {
    ...
    h1sub: "Hamma narsani ko'rasiz. Real vaqtda.",
    sub: "ARKANA ning o'z ITSM platformasi. Har bir ariza, har bir aktiv, har bir SLA ko'rsatkichi — doimo ko'z oldingizda. Endi «ko'rib chiqamiz» va «ertaga qo'ng'iroq qiling» yo'q.",
            ...
            <div style={{ display: "flex", gap: 16, justifyContent: "center", flexWrap: "wrap", marginBottom: 32 }}>
              {c.features.map((item) => (
                <div key={item} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, color: "var(--ark-text-muted)" }}>
                  <CheckCircle2 size={13} style={{ color: "var(--ark-text-muted)" }} />
                  {item}
                </div>
              ))}
            </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {c.modules[active].features.map((f) => (
                  <div key={f} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <CheckCircle2 size={12} style={{ color: "var(--ark-text-muted)" }} />
